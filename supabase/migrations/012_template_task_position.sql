-- Migration 012: update create_campaign_from_template RPC to set task position.
-- Migration 011 added the position column but the RPC wasn't updated; tasks
-- created from template all defaulted to position=0, which breaks drag-and-drop.
      ...
      FOR v_group IN SELECT * FROM jsonb_array_elements(v_template.default_task_groups) LOOP
        ...
        FOR v_task IN SELECT * FROM jsonb_array_elements(v_group->'tasks') LOOP
          INSERT INTO tasks (
            ...
            priority, status, due_date, position, created_by
          ) VALUES (
            v_campaign.id,
            v_group_id,
            v_task->>'title',
